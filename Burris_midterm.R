...
# Prompt: Dr. Smith happens to like dark chocolate brownies and especially favors those that have the
# right amount of chewiness and flavor. Suppose that chewiness can be measured (perhaps by
# eating some). To do this you decide to run an experiment by baking a number of batches of brownies and
# compare them to come up with the best brownies. There were four different brands that were considered. 
# In his study he made a batch of each brownies (using 4 8x8 pans, each pan having a different brand mix),
# set the oven temperature to 350 degrees and placed all four in the oven at once. 
# This was repeated 10 times on different days. 

# Analyze the data – what questions/hypotheses might you ask prior to experimentation?
...
# The results indicate no significant difference in chewiness. 
# However, visualizing the data tells us that the variance amongst each brand is very high.
...
# Tukey comparisons indicates brands 1 and 2 are not significantly difference, 
#  but brand 4 is significantly different from 1 and 2. 
# Brand 3 is not significantly different from any of the other brands. 
